...
try:
    ...
    print('\n这个ini文件被你写“死”了，使用“【素人】影片信息整理工具.exe”重新创建ini！')
    os.system('pause')
...
while 1:
    ...
    only_titleg = re.search(r'(.+?) (.+)', title_easy) #这边匹配番号，[a-z]可能很奇怪，
    nfo_dict['标题'] = only_titleg.group(2)
    nfo_dict['车牌'] = only_titleg.group(1)
    ...
    if str(directorg) != 'None':
        ...
        nfo_dict['导演'] = '未知导演'
    # 演员们 和 # 第一个演员
    actors_prag = re.search(r'<td class="header">演员:(.+?)</td>', javlib_html, re.DOTALL)
    ...
    nfo_dict['标题'] = nfo_dict['标题'].replace('\\', '#').replace('/', '#').replace(':', '：') \
        .replace('*', '#').replace('?', '？').replace('"', '#').replace('<', '【').replace('>', '】').replace('|','#').rstrip(
        nfo_dict['首个女优'])
    # arzon的简介
    plot = ''
